...
%MEAD MAIZE
% for dynamic SLA
Sl_L = [0.036]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC] - simone had it at 0.042
SLA_ref = Sl_L; % reference SLA
boost = 3; % boost when leaf age is 0
age_max = 60; %days - e folding parameter time scale

% %example
% age = 0:300;
% SLA_mod = SLA_ref*(1+boost*exp(-age/age_max));
% set(figure(1), 'color', 'w')
% plot(age, SLA_mod, 'k');
% hold on
% plot(age, SLA_ref*ones(size(age)), 'r--')
% xlabel('Days')
% ylabel('SLA')
% legend({'Modifed SLA', 'Reference SLA'})
% 
...
CT_H=[4]; %%--> 'CT' == 3  'CT' ==  4  %% Photosyntesis Typology for Plants
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%------
FI_L=[0.055];% Intrinsec quantum Efficiency [umolCO2/umolPhotons] ?% The quantum efficiency α was set to the value from de Vries et al. (1989) of 0.055 µmol C m−2s−1
Do_L=[800]; %%[Pa]
a1_L=[4];
go_L=[0.01];% % [mol / s m^2] minimum Stomatal Conductance
CT_L=[4];  %%--> 'CT' == 3  'CT' ==  4  %% Photosyntesis Typology for Plants
...
Psi_sto_50_L = -2.8;%  %% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_L =  -1.2; %% [MPa]  Water Potential at PLCs% loss conductivity
PsiL50_L =  -3.5 ;%%[MPa]  Water Potential at 50% loss conductivity
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Sl_L = [0.036]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC] - simone had it at 0.042 - from lit mine is 0.0648 - before was 0.32
...
aSE_L= [5]; %%% Plant Type -- 1 Seasonal Plant --  0 Evergreen  -- 2 Grass species -- 3 Crops
dd_max_L= [1/50];%%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_L = [7/365]; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_L = [14]; %% [°C] Cold Leaf SLed
drn_L=  [1/365]; %% turnover root  [1/d]
dsn_L= [ 0]; % normal transfer rate sapwood [1/d]
age_cr_L= [60]; %% [day] Critical Leaf Age
Bfac_lo_L= [0.99]; %% Leaf Onset Water Stress
Bfac_ls_L= [NaN]; %% Leaf Shed Water Stress [0-1]
Tlo_L = [18]; %% Mean Temperature for Leaf onset
Tls_L = [NaN]; %% Mean Temperature for Leaf Shed
PAR_th_L= [NaN]; 
dmg_L= [20*0.7]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_L = [0.01];
Trr_L = [4.5]; %% Translocation rate [gC /m^2 d]
mjDay_L = [200]; %% Maximum Julian day for leaf onset
LDay_min_L =[14.3]; %% Minimum Day duration for leaf onset
LtR_L = [1.2]; %%% Leaf to Root ratio maximum
Mf_L= [0]; %% fruit maturation turnover [1/d]
Wm_L= [0] ; % wood turnover coefficient [1/d]
eps_ac_L = [0.2]; %% Allocation to reserve parameter [0-1]
LDay_cr_L = [13.8]; %%%  Threshold for senescence day light [h]
Klf_L =  1/20 ; % [1/83]; %% Dead Leaves fall turnover [1/d]
fab_L = 1; %% fraction above-ground sapwood and reserve
fbe_L = 0; %% fraction below-ground sapwood and reserve
ff_r_L= [0.3];
soCrop_L = [0.05];
MHcrop_L=[2.5];
...
%%%%%%%%%%%%%%%	
Mpar_L(1).Date_sowing = datenum(2001:1:2013,5,19,10,0,0) ; 

Mpar_L(1).Date_sowing = [datenum(2001,5,10,10,0,0) datenum(2002,5,9,10,0,0)...
    datenum(2003,5,15,10,0,0) datenum(2004,5,10,3,0,0)...
    datenum(2005,5,4,10,0,0) datenum(2006,5,5,10,0,0)...
    datenum(2007,5,1,10,0,0) datenum(2008,4,29,10,0,0)...
    datenum(2009,4,20,10,0,0) datenum(2010,4,19,10,0,0)...
    datenum(2011,5,18,10,0,0) datenum(2012,4,24,10,0,0)...
    datenum(2013,4,29,10,0,0) datenum(2014,4,21,10,0,0)...
    datenum(2015,4,30,10,0,0) datenum(2016,5,19,10,0,0)...
    datenum(2017,4,27,10,0,0) datenum(2018,5,8,10,0,0)...
    datenum(2019,4,19,10,0,0) datenum(2020,4,20,10,0,0)];


Mpar_L(1).Date_harvesting = datenum(2001:1:2013,10,5,10,0,0) ; %%% Fully Harvested

Mpar_L(1).Date_harvesting = [datenum(2001,10,18,10,0,0) datenum(2002,11,4,10,0,0)...
    datenum(2003,10,27,10,0,0) datenum(2004,10,15,3,0,0)...
    datenum(2005,10,15,10,0,0) datenum(2006,10,5,10,0,0)...
    datenum(2007,11,5,10,0,0) datenum(2008,11,18,10,0,0)...
    datenum(2009,11,9,10,0,0) datenum(2010,9,21,10,0,0)...
    datenum(2011,10,26,10,0,0) datenum(2012,10,10,10,0,0)...
    datenum(2013,10,22,10,0,0) datenum(2014,10,28,10,0,0)...
    datenum(2015,10,26,10,0,0) datenum(2016,9,22,10,0,0)...
    datenum(2017,10,25,10,0,0) datenum(2018,10,31,10,0,0)...
    datenum(2019,11,4,10,0,0) datenum(2020,9,17,10,0,0)];

Mpar_L(1).Crop_B=[12 12]; 	
Mpar_L(1).Crop_type = ones(1,13); 	
Mpar_L(1).Crop_root = 900*ones(1,13);  	
Mpar_L(1).Crop_crown = 1.0*ones(1,13);  


Mpar_L(1).NPK_res_ini = [5 5 5]; 
Mpar_L(1).Crop_root = 950*ones(size(Mpar_L(1).Date_sowing)) ; 
Mpar_L(1).Crop_crown = ones(size(Mpar_L(1).Date_sowing ));
...
Vmax_H = [0]; %55
Vmax_L = [50]; %35 - Simone had it at 92 - I got 61 from TRY database - was 35 before
%%%%%%%%%%%%%%%%%%%%%%
L_day=zeros(NNd,1);
for j=2:24:NN
