...
%%%%%%%%%%%%%%%%% OTHER PARAMETER
In_max_urb=0.5; In_max_rock=0.1; %% [mm]
%%%%%%%%%%%%% SNOW PARAMETER
...
Do_H=2000; %%[Pa]
a1_H=9; % influences transpiration - latent heat
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
FI_L=0.040;% Intrinsec quantum Efficiency [umolCO2/umolPhotons]
Do_L=2000; %%[Pa]
a1_L=5;
...
rjv_L=2.1; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% Leaf
PsiL00_H =  -2.8; %% [MPa]  Water Potential at 2% loss conductivity
PsiL50_H =  -4.5 ;%%[MPa]  Water Potential at 50% loss conductivity
...
Psi_sto_50_L = -1.6;%  %% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_L =  -2.0; %% [MPa]  Water Potential at PLCs% loss conductivity
PsiL50_L =  -5.5 ;%%[MPa]  Water Potential at 50% loss conductivity
...
PsiX50_L = -5.5; %%[MPa]  Water Potential at 50% loss conductivity
...
[PFT_opt_L(1)]=Veg_Optical_Parameter(14);
OM_H=1;
OM_L=1;
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L=0.020; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
Tlo_L=3.5; %% Mean Temperature for Leaf onset
Tls_L=NaN; %% Mean Temperature for Leaf Shed
PAR_th_L= NaN; 
dmg_L=20; %%% Tree 30 Grasses Day of Max Growth
LAI_min_L=0.1;
Trr_L=3.0; %% Translocation rate [gC /m^2 d]
mjDay_L=250; %% Maximum Julian day for leaf onset
LDay_min_L =10.0; %% Minimum Day duration for leaf onset
LtR_L=0.40; %%% Leaf to Root ratio maximum
Mf_L=1/20; %% fruit maturation turnover [1/d]
Wm_L=0; % wood turnover coefficient [1/d]
eps_ac_L=0.2; %% Allocation to reserve parameter [0-1]
LDay_cr_L=11.4 ; % 12.75; %%%  Threshold for senescence day light [h]
...
if OPT_SoilBiogeochemistry == 1
    %%%%%
    Wm_H= [1/20075] ; % 55 yr wood turnover coefficient [1/d]
    B_H(1,1,6)=20000; %%% wood pool 
    %%%%
    Nreserve_H(1,:)= [2];
    Preserve_H(1,:)= [0.15];
    Kreserve_H(1,:)= [0];
    FNC_H(1,:)=[1 ];
    NupI_H(1,:)= [ 0.0315865   0.0020633  0.022415];
    RexmyI(1,:)= [ 0.023316   0.2100                  0];
    NavlI(1,:)=[   0.118 0.00793  0.0960];
    Nreserve_L(1,:)= [0]; Preserve_L(1,:)= [0]; Kreserve_L(1,:)= [0]; FNC_L(1,:)=[1];
    %%%%
    P(1,:)=  1000*[  0.016354449654662   0.062800703234896   0.010058357195034   0.589424889605380   0.196474963201793   0.009787352305096   0.079761238374483,...
   0.018829489389154   1.312772842249553   0.587026893845305   3.826614162948153   0.014327838996357   0.011578000146098   0.000114647398423,...
   0.000079753563106   0.000047769749343   0.000132922605177   0.022725730194175   0.093408615965358   0.057848337825887                   0,...
   0.006001005601372   0.000858305541971   0.002998828385712   0.000735913337683   0.454614109714426   0.004383466141997   0.014406002879058,...
   0.003207730561364                   0   0.000148264119148   0.000026839439874   0.000017774532203   0.000600100560138   0.000104991586940,...
   0.000214202027551   0.000075386892887   0.070217682414946   0.001417133036320   0.002341012472407   0.000481159584205                   0,...
   0.000050409387982   0.150000000000000   0.043861131298916   0.015010535804291   0.000000179160399   0.000578451650073   0.001499414192856,...
   0.000121704367227   0.005039618116375   0.000101226119829   0.000083225565950   0.001305403645096   0.502968233317084];
